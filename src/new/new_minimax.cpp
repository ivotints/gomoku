...
int new_minimax(move_t &move, bool turn, int alpha, int beta, int depth)
{
    if (depth == 0 || move.eval < -100'000 || move.eval > 100'000) // means that it is a winning move
        return (move.eval);
    ...
    if (turn == BLACK) // maximizing player
    {
        ...
        {
            int eval = new_minimax(moves[i], !turn, alpha, beta, depth - 1);
            if (eval > best_eval)
            {
...
int new_bot_play(uint32_t (&boards)[2][19], bool turn, uint8_t (&captures)[2], int depth)
{
    move_t moves[300];
    short move_count = 0;

    int current_board_eval = bitwise_heuristic(boards[turn], boards[!turn], captures[turn], captures[!turn]); // easier to get initial eval from it, and later to use star_heuristic. But we also can pass this value from gomoku class in python.
    ...
    if (turn == BLACK) // maximizing player
    {
        ...
        for (short i = 0; i < move_count; ++i) // for move in moves
        {
            int eval = new_minimax(moves[i], !turn, -1'000'000, 1'000'000, depth);
            if (eval > best_eval)
            {
