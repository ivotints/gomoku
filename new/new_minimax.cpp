...
void cleanup_thread_local_table() {
    delete[] thread_local_table;
    thread_local_table = nullptr;
}
...
int new_minimax(move_t &move, bool turn, int alpha, int beta, int depth, int &total_evaluated, double &total_time, move_t *moves_last, short move_count_last)
{
    ...
    if (turn == BLACK) // maximizing player
    {
        ...
        {
            if (!is_legal_lite(moves[i].captures[turn], moves[i].boards[turn], moves[i].boards[!turn], moves[i].y, moves[i].x) || isPositionVisited(thread_local_table, moves[i].hash))
                continue;
...
BotResult new_bot_play(uint32_t (&boards)[2][19], bool turn, uint8_t (&captures)[2], int depth)
{
    ...
    for (short i = 0; i < move_count; ++i)
    {
        ...
        if ((turn == BLACK && moves[i].eval > 100000) || (turn == WHITE && moves[i].eval < -100000)) {
    ...
    int best_eval = turn == BLACK ? -1'000'000 : 1'000'000;
    for (unsigned int t = 0; t < thread_count; t++) {
        threads.emplace_back([&, t]() {
            init_thread_local_table();
            for (short i = t; i < move_count; i += thread_count)
            {
                ...
                if ((turn == BLACK && eval > best_eval) || (turn == WHITE && eval < best_eval))
                {
